...
const char reverseDir[4] = {'S', 'N', 'E', 'W'};
...
void solve()
{
    ...
    while (!Q.empty())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            nex.manRow += dir[i][0];
            nex.manCol += dir[i][1];
            if (isUreachable(nex.manRow, nex.manCol))
                continue;
            if (nex.manRow == now.boxRow && nex.manCol == now.boxCol)
            {

                nex.boxRow += dir[i][0];
                nex.boxCol += dir[i][1];
                if (isUreachable(nex.boxRow, nex.boxCol))
                    continue;
                ++nex.push;
                if (!isBetter(nex))
                    continue;
